...
const API_URL = import.meta.env.VITE_API_URL + "/getuserscorrecaminos";
const SAVE_USER_URL = "http://10.10.165.61/swservicefac/public/api/saveusercorrecaminos"; // Endpoint para guardar usuario
      ...
      {brands.map((brand, key) => (
        <div key={key} className="grid grid-cols-3 border-b border-stroke py-2.5 dark:border-strokedark sm:grid-cols-4">
          <div className="flex items-center gap-3">
            <p className="hidden sm:block text-black dark:text-white">{brand.name}</p>
          </div>
